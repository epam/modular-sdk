...
from modular_sdk.models.pynamongo.indexes_creator import IndexesExtractor, index_information_to_index_models, iter_comparing, IndexesCreator
...
class TestIndexesExtractor:
    ...
    def test_get_primary(self):
        assert IndexesExtractor().get_primary(MyModel).document == {'name': 'hash_1_sort_-1', 'unique': True, 'key': {'hash': 1, 'sort': -1}}
        assert IndexesExtractor().get_primary(MyModel, unique=False).document == {'name': 'hash_1_sort_-1', 'unique': False, 'key': {'hash': 1, 'sort': -1}}
        assert IndexesExtractor(
            primary_index_name='main',
...
def test_index_information_to_index_models():
    ...
    assert by_name['index2'].document == {
        'key': {'test': 1},
        'name': 'index2'
    }
